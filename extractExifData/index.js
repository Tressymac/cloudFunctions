...
async function readExifData(localFile, datafile){
    ...
    try{
        ...
        if (Object.keys(exifData.gps).length > 0){
            ...
            thumb256_url: `https://storage.googleapis.com/${thumbNailBucketName}/thumb%40256_${datafile.name}`,
            thumb64_url: `https://storage.googleapis.com/${thumbNailBucketName}/thumb%4064_${datafile.name}`
        };
        //use a dot slice method to fix the name of the datafile.name (I need to remove the "image" letter from the url it should be thumb%40256_1677276588229214 not thumb%40256_image1677276588229214)
        return imageObject;
        } else{
